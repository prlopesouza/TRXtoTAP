...
namespace TRX_to_TAP
{
    struct UnitTestResult
    {
        ...
        public List<UnitTestResult> innerResults;
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            navigator.MoveToChild("Results", namespaceUri);
            ...
            while (ok)
            {
                ...
                res.inner = false;
                res.relativeResultsDirectory = navigator.GetAttribute("relativeResultsDirectory", "");
                res.testName = navigator.GetAttribute("testName", "");
                res.outcome = navigator.GetAttribute("outcome", "");
                res.duration = navigator.GetAttribute("duration", "");
                res.ResultFiles = new List<string>();
                res.innerResults = new List<UnitTestResult>();
                if (navigator.MoveToChild("ResultFiles", namespaceUri))
                {
                    navigator.MoveToFirstChild();
                    do
                    {
                        res.ResultFiles.Add(DeploymentPath + "\\In\\" + res.relativeResultsDirectory + "\\" + navigator.GetAttribute("path", ""));
                    } while (navigator.MoveToNext());
                    navigator.MoveToParent();
                    navigator.MoveToParent();
                }
                if (navigator.MoveToChild("Output", namespaceUri))
                {
                    if (navigator.MoveToChild("ErrorInfo", namespaceUri))
                    {
                        if (navigator.MoveToChild("Message", namespaceUri))
                        {
                            res.errorMessage = navigator.InnerXml;
                            navigator.MoveToParent();
                        }
                        if (navigator.MoveToChild("StackTrace", namespaceUri))
                        {
                            res.StackTrace = navigator.InnerXml;
                            navigator.MoveToParent();
                        }
                        navigator.MoveToParent();
                    }
                ...
                if (navigator.MoveToChild("InnerResults", namespaceUri))
                {
                    navigator.MoveToFirstChild();
                    do
                    {
                        UnitTestResult resIn = new UnitTestResult();
                        resIn.inner = true;
                        resIn.relativeResultsDirectory = navigator.GetAttribute("relativeResultsDirectory", "");
                        resIn.testName = navigator.GetAttribute("testName", "");
                        resIn.outcome = navigator.GetAttribute("outcome", "");
                        resIn.duration = navigator.GetAttribute("duration", "");
                        resIn.ResultFiles = new List<string>();
                        resIn.ResultFiles.AddRange(res.ResultFiles);
                        if (navigator.MoveToChild("ResultFiles", namespaceUri))
                        {
                            navigator.MoveToFirstChild();
                            do
                            {
                                resIn.ResultFiles.Add(DeploymentPath + "\\In\\" + resIn.relativeResultsDirectory + "\\" + navigator.GetAttribute("path", ""));
                            } while (navigator.MoveToNext());
                            navigator.MoveToParent();
                            navigator.MoveToParent();
                        }
                        if (navigator.MoveToChild("Output", namespaceUri))
                        {
                            if (navigator.MoveToChild("ErrorInfo", namespaceUri))
                            {
                                if (navigator.MoveToChild("Message", namespaceUri))
                                {
                                    resIn.errorMessage = navigator.InnerXml;
                                    navigator.MoveToParent();
                                }
                                if (navigator.MoveToChild("StackTrace", namespaceUri))
                                {
                                    resIn.StackTrace = navigator.InnerXml;
                                    navigator.MoveToParent();
                                }
                                navigator.MoveToParent();
                            }
            ...
            if (!Directory.Exists(tapRoot))
            {
                Directory.CreateDirectory(tapRoot);
            }

            foreach (UnitTestResult res in results)
            {
                FileStream fs = File.Open(tapRoot + "\\" + res.testName + ".tap", FileMode.Create);
                StreamWriter sw = new StreamWriter(fs);
                int c = res.innerResults.Count;
                if (c == 0)
                {
                    sw.WriteLine("1..1");
                    switch (res.outcome)
                    {
                        case "Passed":
                            ...
                            break;
                        case "Failed":
                            sw.WriteLine("not ok 1");
                            break;
                        case "NotExecuted":
                            sw.WriteLine("not ok 1 # SKIP");
                            break;
                        default:
                ...
                if (c > 0)
                {
                    sw.WriteLine("1.." + c);
                    int i = 1;
                    foreach (UnitTestResult inner in res.innerResults)
                    {
                        switch (inner.outcome)
                        {
                            case "Passed":
                                ...
                                break;
                            case "Failed":
                                sw.WriteLine("not ok " + i);
                                break;
                            case "NotExecuted":
                                sw.WriteLine("not ok " + i + " # SKIP");
                                break;
                            default:
                                sw.WriteLine("not ok " + i + " - " + inner.outcome);
                                break;
                        }
                        if (inner.errorMessage != null)
                        {
                            String erro = inner.errorMessage.Replace("\r\n", "\r\n# ");
                            sw.WriteLine("# Erro: " + erro);
                        }
                        if (inner.StackTrace != null)
                        {
                            String stack = inner.StackTrace.Replace("\r\n", "\r\n# ");
                            sw.WriteLine("# StackTrace: " + stack);
                        }
                        if (inner.ResultFiles.Count > 0)
                        {
                            sw.WriteLine("   ---");
                            sw.WriteLine("   extensions:");
                            sw.WriteLine("    Files:");
                            foreach (String file in inner.ResultFiles)
                            {
